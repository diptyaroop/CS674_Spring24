...
def render_viewpoints(model, render_poses, HW, Ks, render_kwargs,
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            filename = os.path.join(savedir, 'DM_{:03d}.png'.format(i))
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    print("[DM]: checkpoint path: ", reload_ckpt_path)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        # print("[DM] render result: ", render_result)
...
def train(args, cfg, data_dict):
    ...
    with open(os.path.join(cfg.basedir, cfg.expname, 'args.txt'), 'w') as file:
        for arg in sorted(vars(args)):
            ...
            file.write('{} = {}\n'.format(arg, attr))
    cfg.dump(os.path.join(cfg.basedir, cfg.expname, 'config.py')) # [DM] save configs in args.txt file in the log folder
    ...
    print('train coarse: finish (eps time', eps_time_str, ')')

    # [DM] Try to change this to multi-resolution hash encoding
...
if __name__=='__main__':
    ...
    if torch.cuda.is_available():
        ...
        # torch.set_default_dtype('torch.float64') # [DM] changes for Pytorch>=2.1
        # torch.set_default_device('cuda') # [DM] changes for Pytorch>=2.1 
        # # [DM] If the above 2 lines don't work, revert to earlier code which is commented above
    ...
    if args.export_coarse_only:
        ...
        coarse_image_save_path = os.path.join(cfg.basedir, cfg.expname, args.export_coarse_only) # [DM]
        np.savez_compressed(coarse_image_save_path, alpha=alpha, rgb=rgb)
        print("[DM] Image saved to: ", coarse_image_save_path) # [DM]
        print('done')
        sys.exit()
    ...
    if args.render_test or args.render_train or args.render_video:
        if args.ft_path:
            ...
            if (args.eval_coarse_grid): # [DM]
                ckpt_path = os.path.join(cfg.basedir, cfg.expname, 'coarse_last.tar') # [DM]
